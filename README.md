...
Original JEMM already helps with private content that isn’t on public databases. **JEMM-MA** doubles down on that: start (or reset) a single file or even your whole library to zero and build tags, names, people, genres, and studios under your control.

Typical use cases:

- You have a lot of home videos and other private media with no external metadata source
- You want to apply your own custom tagging logic, you define yourself
- You want to clear scraped or leftover metadata so you can start clean

But wait, there's more! **All of the original JEMM functionality remains***. Paired with new batch tools under the **Tools** menu and everything now always works on your library selection (not just the first selected), including their subfolders.
...
- **Auto Tags** – Adds tags from technical media info: orientation (`vertical` / `horizontal` / `square`), frame rate (`low fps` / `standart fps` / `high fps`, videos only), resolution (`SD` / `HD` / `FULL HD` / `2K` / `4K` / `6K` / `8K` / `ULTRA RES`, the same ladder for videos and images), `No Audio` for silent videos, and a linear quality rating (`QR0`, `QR1`, `QR2`, …) from data density per megapixel/bitrate, normalized to the standard 24–30 fps range so low/high-fps clips aren't mis-rated. Each category (orientation, resolution, frame rate, quality rating, no-audio) can be enabled/disabled with checkboxes. Only its own computed tags are added/updated; your manual tags are left alone. When Jellyfin's API doesn't expose the technical data, an optional **ffprobe** fallback reads it directly from the media — see below.
...
- **Tag-Team Mode** – A guided, keyboard-fast pass over the selected libraries that walks user-defined tag/genre decision trees (your "tag map") for each folder and file, with always-on Actors / Studios / Release-date panels and filename-based suggestions. Comes with a GUI editor for your tag map. See below.

#### Tag-Team Mode

Tag-Team Mode turns manuel tagging into a quick, click-through workflow. It walks every **stop** in the selected libraries — each folder, then its files, recursively — and for each stop lets you click through your **tag map** (decision trees) while editing actors, studios and the release date on the side.

- **Tag Map** Edit it in **Tools → Tag Map Editor…** (tree outline + detail form + visual structure preview). JSON is also fully supported via **Tools → Import Tag Map…** / **Export Tag Map…**. The Tag-Team start dialog shows the active map and lets you choose what kind of files should be walked through (e.g. only videos). There can only be a single active tag map at this time.
...
- **Fast walk.** At start, all necessary metadata and catalogs are preloaded. During the walk, applies stay **in memory**; **Finish & Close** (or Save when closing the window) POSTs all pending changes. So no waiting times while hopping from file to file.
...
- **Side panels.** Actors (with a type dropdown), Studios and Release date are always visible, pre-filled from the item, and editable/removable. Entries are listed with their type (`Alice (Actor)`, `Alice (Director)`). Live autocomplete uses the Jellyfin people/studios catalog; Name+Type duplicates are rejected. Suggestions appear inline in a distinct color; click one to promote it.
  -- **Canonical title suggestion.** The suggested Name follows `[Studio1 | Studio2] Actor1, Actor2 - Video title (YYYY-MM-DD)` and rebuilds live as you edit studios, actors (Type=Actor only in the cast segment), core title, and the date field. Click the chip to copy it into *Set title*. Hash-like file titles fall back to the parent folder name.
  -- **Filename suggestions.** Names like `[Studio]actor - title (date)` are parsed into Studio/Actor/Title/Date suggestions; the catalog also reclassifies known data like known studios that would otherwise be treated as actors (e.g. `StudioX - Alice`). Ambiguous dates (e.g. `123022`) are pruned to the valid readings.
...
- **Keybinds.** Number keys pick chips; **Enter** submits an actor and confirms a multi-select.
...
Auto Tags first uses the technical data Jellyfin already exposes (MediaStreams). If that data is missing for an item, it can optionally fall back to **[ffprobe](https://ffmpeg.org/ffprobe.html)** to read the real data from the media.
...
- Input is chosen automatically: the item's **local file path** when it is reachable from the machine running JEMM-MA, otherwise the Jellyfin **download URL**.
- If ffprobe isn't available, Auto Tags simply uses the API-only data and reports it in the summary.
- An optional second checkbox lets you fall back to the **poster/aspect-ratio hint** for orientation only when ffprobe is unavailable. This is a best-effort guess.

### Quality of life & changed behaviour

- **Remembered login.** After a successful login, the Jellyfin server URL and API key are stored and pre-filled on the next start.
...
- **Multi-selection everywhere.** **Apply Changes** in **Library Content** applies to every selected item (upstream handled a single item). Tools and the **Libraries** list act on all selected libraries, including subfolders. When applying to several items, People/Genres/Studios/Tags are now **merged**, not overwritten.
- **Optional fields are truly optional.** Fields that JEMM used to require but Jellyfin does not are no longer mandatory, so now you can save without filling them in. That little thing started this fork.
...
JEMM-MA specific code is 99% generated by AI. 
